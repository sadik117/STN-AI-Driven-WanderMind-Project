  ...
  const navLinks = [
    ...
    { 
      ...
      subLinks: [
        ...
        { href: '/smart-packing', label: 'Smart Packing', icon: Package },
        { href: '/budget-analyzer', label: 'Budget Analyzer', icon: Calculator },
        { href: '/journal-summarizer', label: 'Journal Summarizer', icon: FileText },
      ]
    },
  ];
                ...
                <DropdownMenu key={link.label}>
                  <DropdownMenuTrigger asChild>
                    <motion.div
                      whileHover={{ y: -2 }}
                      whileTap={{ y: 0 }}
                      className={`relative px-4 py-2 rounded-full text-sm font-medium transition-all duration-300 text-muted-foreground hover:text-primary hover:bg-primary/5 cursor-pointer`}
                    >
                  ...
                  <DropdownMenuContent align="center" className="w-64 rounded-2xl p-2 border-border/50 shadow-xl mt-2">
                    {link.subLinks?.map((subLink) => (
                      <DropdownMenuItem key={subLink.href} asChild className="rounded-xl cursor-pointer py-3 hover:bg-primary/5 transition-all focus:bg-primary/5">
                        <Link href={subLink.href} className="flex items-center gap-3 w-full">
                          <div className="bg-primary/10 p-2 rounded-lg flex-shrink-0">
                            <subLink.icon className="h-5 w-5 text-primary" />
                          </div>
                          <div className="flex flex-col">
                            <span className="font-semibold text-sm">{subLink.label}</span>
                          </div>
                        </Link>
                      </DropdownMenuItem>
                    ))}
                ...
                <div className="flex flex-col h-full">
                    ...
                    {isAuthenticated && (
                      <div className="flex items-center gap-3 mt-2 p-3 bg-muted/50 rounded-2xl">
                        ...
                        <div className="space-y-1">
                          ...
                          <p className="text-[10px] text-muted-foreground rounded-full">
                            {user?.role || 'Traveler'}
                          </p>
                          <p className="text-xs text-muted-foreground">{user?.email}</p>
                        </div>
                  ...
                  <div className="flex-1 overflow-y-auto py-4 px-4">
                    <div className="space-y-1">
                      ...
                      {navLinks.map((link) => (
                        link.isDropdown ? (
                          <div key={link.label} className="px-2 py-1">
                            <div className="flex items-center gap-2 px-1 py-2 text-muted-foreground font-semibold uppercase tracking-wider text-xs">
                              <link.icon className="h-4 w-4" />
                              ...
                              {link.subLinks?.map((subLink) => (
                                ...
                                >
                                  <div className="bg-primary/10 p-1.5 rounded-md">
                                    <subLink.icon className="h-4 w-4 text-primary" />
                                  </div>
